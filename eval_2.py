...
class Eval:
    ...
    def format_output(self, batch, output, pred_out_dir):
        batch_boxes, batch_scores = output
        for index in range(batch['image'].size(0)):
            original_shape = batch['shape'][index]
            filename = batch['filename'][index]
            result_file_name = 'res_' + filename.split('/')[-1].split('.')[0] + '.txt'
            result_file_path = os.path.join(pred_out_dir, result_file_name)
        ...
        with torch.no_grad():
            for _, data_loader in self.data_loaders.items():
                ...
                for i, batch in tqdm(enumerate(data_loader), total=len(data_loader)):
                    ...
                    pred_out_dir = os.path.join(self.args['result_dir'], 'pred_out')
                    os.makedirs(pred_out_dir, exist_ok= True)

                    sample_metric_dir = os.path.join(self.args['result_dir'], 'sample_metric')
                    os.makedirs(sample_metric_dir, exist_ok= True)

                    try: 
                        self.format_output(batch, output, pred_out_dir)
                        raw_metric, interested = self.structure.measurer.validate_measure(batch, output, is_output_polygon=self.args['polygon'], box_thresh=self.args['box_thresh'])
                        ...
                        focused_metrics = {
                            'precision': raw_metric[0]['precision'],
                            'recall': raw_metric[0]['recall'],
                            'hmean': raw_metric[0]['hmean'],
                        }
                        filename = batch['filename'][0]
                        sample_metric_file = os.path.join(sample_metric_dir, f'{filename}.json')
                        with open(sample_metric_file, 'w') as f:
                            json.dump(focused_metrics, f, indent=2)
                ...
                for key, metric in metrics.items():
                    res_str = '%s : %f (%d)' % (key, metric.avg, metric.count)
                    self.logger.info(res_str)
                    overall_res.append(res_str)

                with open(os.path.join(self.args['result_dir'], 'overall_metric.txt'), 'w') as f:
                    f.write('\n'.join(overall_res))
